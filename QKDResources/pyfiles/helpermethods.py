from ipywidgets import Layout, Button, Box, widgets, VBox, HBox, interact, Label, Output
...
def newfillblank(question, answer, layouttouse):
    qoutput = Output()
    with qoutput:
        display(Math(question))
    textBox = widgets.Text(placeholder="a + bi", disabled=False, layout=layouttouse)
...
def newfloatbox(question, answer, layouttouse):
    qoutput = Output()
    with qoutput:
        display(Math(question))
...
def checkfloat(answer, key, validcheck, labeltoreturn):
    try:
        if math.isclose(answer, key, rel_tol=0.005):
            qcorrect(validcheck, labeltoreturn)
            return 1
        else:
            qincorrect(validcheck, labeltoreturn)
...
def checkfloatpair(answer1, answer2, key, validcheck, labeltoreturn):
    try:
        if math.isclose(answer1, key[0], rel_tol=0.005) and math.isclose(
            answer2, key[1], rel_tol=0.005
        ):
            qcorrect(validcheck, labeltoreturn)
            return 1
        else:
...
def printQuesion(question, size):
    ...
    toprint = VBox(
        [
            VBox(
                [
                    HBox([question[8], Label()], layout=Layout(height="40px")),
                    VBox(
                        [question[1]],
                ...
                layout=Layout(
                    height=str(size + 40) + "px", border="solid 1px", width="75%"
                ),
            ),
            HBox(
                [
...
q1 = prepareQuestion(
    ...
    "<b>LHS:</b> $(A+B)^T=\\begin{bmatrix}i&2+5i\\\\3+11i&6-10i\end{bmatrix}^T$$=\\begin{bmatrix}i& 3+11i\\\\2+5i&6-10i\end{bmatrix}$",
)
q2 = prepareQuestion("2. $(A+B)^T = A^T+B^T$", ["hint 1"], "answerzzz")
q3 = prepareQuestion(
    "3. $(A+B)^T = A^T+B^T$", ["hint 1", "hint 2", "hint 3"], "answerzzz"
)
