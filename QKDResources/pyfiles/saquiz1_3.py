...
numInputLayout2 = Layout(width="60px")
...
# Text
probstatment = widgets.HTML(
    value='<font size="+1">Solve 5 randomly generated problems in a row.'
)
...
exercise2 = VBox(
    [
        ...
        Label(),
        VBox(
            [HBox([btn_2_check, Label(), btn_2_restart])],
...
form_items = [
    VBox(
        [
            Label(),
            widgets.HTML(value='<font size="+2">Complete the exercises.</font>'),
            Label(),
        ],
        layout=Layout(align_items="center"),
    ),
    HBox(
        [
            widgets.HTML(value="", layout=Layout(width="15%")),
            widgets.HTML(value="", layout=Layout(width="70%")),
        ],
...
def nextifneeded():
    ...
    if btn_2_check.description[6] != "5":
        btn_2_check.description = (
            "Check " + str(int(btn_2_check.description[6]) + 1) + "/5"
        )
...
def checkAnswers_2(btn):
    ...
    while True:
        ...
        if len(questions) == 2:
            answer = complex(questions[0]) - complex(questions[1])
            break
    if (cmath.isclose(answer.real, intbox2_1.value)) and (
        cmath.isclose(answer.imag, intbox2_2.value)
    ):
        nextifneeded()
    else:
        ...
        probstatment.value = (
            '<font size="+1"> The correct answer was: ('
            + str(answer.real)
            + " + "
            + str(answer.imag)
            + "i)"
